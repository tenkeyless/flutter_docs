---
# title: Cookbook
title: 쿡북
description: >
  Flutter 쿡북은 흔히 수행되는 다양한 작업에 대한 레시피를 제공합니다.
---

이 쿡북에는 Flutter 앱을 작성하는 동안 일반적인 문제를 해결하는 방법을 보여주는 레시피들이 들어 있습니다. 
각 레시피는 독립적(self-contained)이고, 애플리케이션을 빌드하는 데 도움이 되는 참조로 사용할 수 있습니다.

## 애니메이션 {:#animation}
- [페이지 경로(route) 전환 애니메이팅](/cookbook/animation/page-route-animation)
- [물리 시뮬레이션을 사용하여 위젯 애니메이션 만들기](/cookbook/animation/physics-simulation)
- [컨테이너의 속성에 애니메이팅](/cookbook/animation/animated-container)
- [위젯을 페이드 인 및 페이드 아웃](/cookbook/animation/opacity-animation)


## 디자인 {:#design}
- [화면에 Drawer 추가](/cookbook/design/drawer)
- [스낵바 표시](/cookbook/design/snackbars)
- [패키지에서 글꼴 내보내기](/cookbook/design/package-fonts)
- [방향(orientation)에 따라 UI 업데이트](/cookbook/design/orientation)
- [커스텀 글꼴 사용](/cookbook/design/fonts)
- [테마를 사용하여 색상 및 글꼴 스타일 공유](/cookbook/design/themes)
- [탭으로 작업하기](/cookbook/design/tabs)


## 효과 {:#effects}
- [다운로드 버튼 만들기](/cookbook/effects/download-button)
- [중첩된 네비게이션 흐름 만들기](/cookbook/effects/nested-nav)
- [사진 필터 carousel 만들기 (deprecated)](/cookbook/effects/photo-filter-carousel)
- [스크롤링 패럴랙스 효과 만들기](/cookbook/effects/parallax-scrolling)
- [반짝이는(shimmer) 로딩 효과 만들기](/cookbook/effects/shimmer-loading)
- [단계적(staggered) 메뉴 애니메이션 만들기](/cookbook/effects/staggered-menu-animation)
- [타이핑 표시기 만들기 (deprecated)](/cookbook/effects/typing-indicator)
- [확장 가능한 FAB 만들기](/cookbook/effects/expandable-fab)
- [그라데이션 채팅 버블 만들기 (deprecated)](/cookbook/effects/gradient-bubbles)
- [UI 요소 드래그](/cookbook/effects/drag-a-widget)


## 폼 {:#forms}
- [유효성 검증이 있는 양식 만들기](/cookbook/forms/validation)
- [텍스트 필드 만들기 및 스타일 지정](/cookbook/forms/text-input)
- [포커스 및 텍스트 필드](/cookbook/forms/focus)
- [텍스트 필드의 변경 사항 처리](/cookbook/forms/text-field-changes)
- [텍스트 필드의 값 검색](/cookbook/forms/retrieve-input)


## 게임 {:#games}

- [모바일 게임에 업적과 리더보드 추가](/cookbook/games/achievements-leaderboard)
- [Firestore를 통해 멀티플레이어 지원 추가](/cookbook/games/firestore-multiplayer)
- [모바일 Flutter 앱 또는 게임에 광고 추가](/cookbook/plugins/google-mobile-ads)


## 제스쳐 {:#gestures}
- [Material 터치 리플(ripples) 추가](/cookbook/gestures/ripples)
- [탭 다루기](/cookbook/gestures/handling-taps)
- [스와이프하여 사라지기 구현](/cookbook/gestures/dismissible)


## 이미지 {:#images}
- [인터넷으로부터 이미지 표시](/cookbook/images/network-image)
- [플레이스홀더로 이미지 페이드 인](/cookbook/images/fading-in-images)


## 리스트 {:#lists}
- [그리드 목록 만들기](/cookbook/lists/grid-lists)
- [horizontal 목록 만들기](/cookbook/lists/horizontal-list)
- [다양한 유형의 항목으로 목록 만들기](/cookbook/lists/mixed-list)
- [목록 위에 떠 있는 앱 바 배치](/cookbook/lists/floating-app-bar)
- [목록 사용](/cookbook/lists/basic-list)
- [긴 목록으로 작업하기](/cookbook/lists/long-lists)
- [간격을 둔 항목으로 목록 만들기](/cookbook/lists/spaced-items)


## 유지 {:#maintenance}
- [서비스에 오류 보고](/cookbook/maintenance/error-reporting)


## 네비게이션 {:#navigation}
- [화면 간에 위젯 애니메이션](/cookbook/navigation/hero-animations)
- [새 화면으로 이동하고 돌아가기](/cookbook/navigation/navigation-basics)
- [이름이 있는 routes로 이동](/cookbook/navigation/named-routes)
- [이름이 있는 routes에 인수 전달](/cookbook/navigation/navigate-with-arguments)
- [Android 용 앱 링크 설정](/cookbook/navigation/set-up-app-links)
- [iOS 용 universal 링크 설정](/cookbook/navigation/set-up-universal-links)
- [화면으로부터 ​​데이터 반환](/cookbook/navigation/returning-data)
- [새 화면으로 데이터 보내기](/cookbook/navigation/passing-data)



## 네트워킹 {:#networking}
- [인터넷으로부터 데이터 가져오기](/cookbook/networking/fetch-data)
- [인증된 요청 만들기](/cookbook/networking/authenticated-requests)
- [인터넷으로 데이터 보내기](/cookbook/networking/send-data)
- [인터넷에서 데이터 업데이트](/cookbook/networking/update-data)
- [인터넷에서 데이터 삭제](/cookbook/networking/delete-data)
- [WebSockets으로 통신하기](/cookbook/networking/web-sockets)
- [백그라운드에서 JSON 구문 분석하기](/cookbook/networking/background-parsing)


## 저장 {:#persistence}
- [SQLite로 데이터 유지](/cookbook/persistence/sqlite)
- [파일 읽기 및 쓰기](/cookbook/persistence/reading-writing-files)
- [키-값 데이터를 디스크에 저장](/cookbook/persistence/key-value)


## 플러그인 {:#plugins}

- [비디오 재생 및 일시 정지](/cookbook/plugins/play-video)
- [모바일 Flutter 앱 또는 게임에 광고 추가](/cookbook/plugins/google-mobile-ads)
- [카메라를 사용하여 사진 촬영](/cookbook/plugins/picture-using-camera)


## 테스트 {:#testing}

### 통합 {:#integration}
- [통합 테스트 소개](/cookbook/testing/integration/introduction)
- [성능 프로파일링](/cookbook/testing/integration/profiling)

### 유닛 {:#unit}
- [단위 테스트 소개](/cookbook/testing/unit/introduction)
- [Mockito를 사용하여 종속성 Mock](/cookbook/testing/unit/mocking)


### 위젯 {:#widget}
- [위젯 테스트 소개](/cookbook/testing/widget/introduction)
- [위젯 찾기](/cookbook/testing/widget/finders)
- [스크롤 처리](/cookbook/testing/widget/scrolling)
- [탭, 드래그, 텍스트 입력](/cookbook/testing/widget/tap-drag)
