...
이 메서드는 `String` 유형의 매개변수를 받습니다. `hello` 메서드는 문자열을 반환합니다. 
...
| `preferred-supported-locales`       | 애플리케이션에 대한 기본 지원 로케일 리스트입니다. 기본적으로, 도구는 지원되는 로케일 목록을 알파벳순으로 생성합니다. 이 플래그를 사용하여 다른 로케일로 기본 설정합니다.<br /><br />예를 들어, 장치가 지원하는 경우 `[ en_US ]`를 전달하여 미국 영어로 기본 설정합니다. |
